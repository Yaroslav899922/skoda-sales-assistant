...
const App: React.FC = () => {
  ...
  const handleStartAnalysis = async (files: File[], details: CarDetails) => {
    ...
    try {
      ...
      setError(err.message || "Помилка аналізу. Перевірте з'єднання.");
  ...
  const handleGenerateAds = async () => {
    ...
    try {
      ...
      setError(err.message || "Помилка генерації текстів.");
      setAppState(AppState.REVIEW);
    }
  };
       ...
       <div className="glass-panel rounded-2xl overflow-hidden border border-white/60 shadow-lg">
          ...
          {images.length > 1 && (
            <div className="p-4 bg-white/40 border-t border-white/50 flex gap-2 overflow-x-auto">
                {images.map((img, i) => (
                  <button key={i} onClick={() => setActiveImageIndex(i)} className={`w-12 h-12 flex-shrink-0 rounded border-2 ${i === activeImageIndex ? 'border-skoda-emerald' : 'border-transparent'}`}>
  ...
  return (
    <div className="flex flex-col min-h-screen relative bg-[#F9FBFB]">
      ...
      <main className="flex-grow max-w-7xl mx-auto w-full px-4 py-8 relative z-10">
        {error && (
          <div className="mb-6 p-4 glass-panel border-l-4 border-red-500 text-red-700 flex justify-between items-center rounded-xl">
            <span>{error}</span>
        ...
        {appState === AppState.IDLE ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-fade-in">
              <div className="lg:col-span-4"><LeftColumnContent /></div>
